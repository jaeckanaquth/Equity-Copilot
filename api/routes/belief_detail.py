"""Belief detail, review outcome, decision, confidence, cadence, and return-observation routes."""
...
from core.models.reasoning_artifact import ArtifactType, ConfidenceLevel, ReasoningArtifact
...
def record_review_outcome(
    ...
    if cadence_row and cadence_row.cadence_days is not None:
        ...
        days = int(cadence_row.cadence_days) if cadence_row.cadence_days is not None else None
        if days is not None:
            cadence_repo.set(belief_id, today + timedelta(days=days), days)
    return None
